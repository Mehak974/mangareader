...
const REFERERS = {
  ...
  'hadesscans.com': 'https://hadesscans.com',
  'isekaiscans.org': 'https://isekaiscans.org/',
...
const SOURCE_SCRAPERS = {
  ...
  hadesscans: {
    ...
    async getHome() {
      const html = await fetchHTML('https://hadesscans.com/');
      const $ = cheerio.load(html);
      const results = [];

      // HadesScans card grid using new class selectors
      $('.cx-poster-card, .bixbox, .animposx, .bs, .bsx').each((_, el) => {
        const $el = $(el);
        const $a = $el.find('a').first().length ? $el.find('a').first() : $el;
        const href = $a.attr('href') || '';
        const title = $el.find('.cx-poster-card__title, h3, h4, .tt, .bixtitle, h2').first().text().trim() ||
          $a.attr('title') || '';
        const cover = $el.find('img').first().attr('src') || $el.find('img').first().attr('data-src') || '';
        const chapter = $el.find('.rating, .numscore').text().trim() || '';

        if (title && href) {
          results.push({ title, href: toAbsolute(href, 'https://hadesscans.com'), cover, chapter });
        }
      });

      return { section: 'New Releases', items: dedupByHref(results).slice(0, 12) };
    },
    ...
    async getMangaDetail(url) {
      ...
      if (nextDataEl.length) {
        try {
          ...
          if (props.manga || props.series) {
            const manga = props.manga || props.series;
            return {
              ...
              status: manga.status || '',
              genres: manga.genres || [],
              chapters: (manga.chapters || []).slice(0, 50).map(c => ({
                title: `Chapter ${c.number || c.chapter}`,
                href: toAbsolute(c.url || '', 'https://hadesscans.com'),
              })),
            };
          }
        } catch (_) { }
      ...
      const cover = $('.thumb img, .seriesimg img').attr('src') || '';
      const description = $('.summary p, .entry-content p').first().text().trim();
      const status = $('.status i, .imptdt i').text().trim();
      const genres = [];
      $('.mgen a, .genre-content a').each((_, el) => genres.push($(el).text().trim()));

      const chapters = [];
      '#chapterlist li a, .eplister li a, .cx-chapter-list a, a[href*="-chapter-"]'.split(', ').forEach(sel => {
        $(sel).each((_, el) => {
          const href = $(el).attr('href') || '';
          let chTitle = $(el).find('[class*="title"], span').first().text().trim() || $(el).text().trim();
          chTitle = chTitle.split('\n')[0].trim();
          if (chTitle.toLowerCase().includes('read') && chTitle.length > 25) {
            chTitle = chTitle.replace(/Latest|Read/gi, '').trim();
          }
          const date = $(el).closest('li').find('.epl-date, .chapterdate, .chapter-release-date').first().text().trim() || null;
          if (href && !chapters.some(c => c.href === href)) {
            chapters.push({ title: chTitle, href: toAbsolute(href, 'https://hadesscans.com'), date });
          }
        });
      });
    ...
    async getChapterImages(url) {
      ...
      let images = strategy2_nextData($);

      // Strategy 2: Embedded JSON scripts
      if (images.length === 0) images = strategy1_embeddedJSON($);

      // Strategy 3: DOM selectors
      if (images.length === 0) images = strategy3_domSelectors($);

      return { images, source: 'hadesscans' };
    }
  },
  ...
  isekaiscans: {
    id: 'isekaiscans',
    name: 'Isekai Scans',
    baseUrl: 'https://isekaiscans.org',
    color: '#34495e',

    async getHome() {
      const html = await fetchHTML('https://isekaiscans.org/');
      const $ = cheerio.load(html);
      const results = [];

      $('.listupd .bsx, .listupd .utao').each((_, el) => {
        const $el = $(el);
        ...
        if (title && href) {
          results.push({ title, href: toAbsolute(href, 'https://isekaiscans.org'), cover, chapter });
        }
      });

      return { section: 'Latest Releases', items: dedupByHref(results).slice(0, 12) };
    },
    ...
    async getMangaDetail(url) {
      ...
      $('a').each((_, el) => {
        const href = $(el).attr('href') || '';
        let chTitle = $(el).text().trim().replace(/\s+/g, ' ');
        if (href.includes('-chapter-') && chTitle) {
          chTitle = chTitle.split('\n')[0].trim();
          if (chTitle.toLowerCase().includes('read') && chTitle.length > 25) {
            chTitle = chTitle.replace(/Latest|Read/gi, '').trim();
          }
          const date = $li.find('.chapter-release-date').first().text().trim() || null;
          if (!chapters.some(c => c.href === href)) {
            chapters.push({ title: chTitle, href: toAbsolute(href, 'https://isekaiscans.org'), date });
          }
        }
      });
    ...
    async getChapterImages(url) {
      ...
      if (images.length === 0) images = strategy3_domSelectors($);
      return { images, source: 'isekaiscans' };
    }
  },
  ...
  mgeko: {
    ...
    async getHome() {
      const html = await fetchHTML('https://www.mgeko.cc/');
      const $ = cheerio.load(html);
      const results = [];
      ...
      $('.novel-item').each((_, el) => {
        const $el = $(el);
        const $a = $el.find('a').first();
        const title = $a.attr('title') || $el.find('.novel-title').text().trim() || '';
        const href = $a.attr('href') || '';
        const cover = $el.find('img').attr('data-src') || $el.find('img').attr('src') || '';

        if (title && href && href.includes('/manga/')) {
          results.push({ title, href: toAbsolute(href, 'https://www.mgeko.cc'), cover, chapter: '' });
        }
      });

      return { section: 'Popular Updates', items: dedupByHref(results).slice(0, 12) };
    },
    ...
    async getMangaDetail(url) {
      ...
      const cover = $('.cover img, img.cover, .manga-info-pic img').first().attr('src') || '';
      const description = $('.description, .synopsis, .summary').first().text().trim();
      const status = $('.status, .info-status').first().text().trim();
      const genres = [];
      $('.genre a, .tag-links a').each((_, el) => genres.push($(el).text().trim()));
      ...
      $('a').each((_, el) => {
        ...
        if (href.includes('/reader/')) {
          ...
          chapters.push({ title: chTitle, href: toAbsolute(href, 'https://www.mgeko.cc') });
        }
      });
    ...
    async getChapterImages(url) {
      ...
      const finalImages = images.length === 0 ? strategy3_domSelectors($) : images;
      return { images: finalImages, source: 'mgeko' };
    }
  },
